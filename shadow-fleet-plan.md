...
24. Analyst console (ADR-18, decided by Adam Sep 17): a local React console replaces the "Streamlit map" stretch item and the "no web UI beyond Streamlit" cut. It is a static app over a JSON bundle that Python writes; its data contract is frozen now and tested from both sides, so backend phases keep moving and the console is wired to live data in one session after Phase 6 (Phase C). It is built against a synthetic fixture until then. It costs about one session of the week 11 to 12 buffer.
...
4. ~~Streamlit map of a flagged vessel's timeline.~~ Superseded by the analyst console (ADR-18), wired to live data in Phase C. Optional later screens: brief viewer with clickable evidence ids (after Phase 8), and an in-app FP review / audit sheet (Phases 6 and 9).

### Cut
GNNs (GraphSAGE and friends): the graph has a few thousand nodes and sparse labels; PPR features will capture most of the signal and a GNN would consume two phases for a result the evaluation cannot distinguish from noise. RAG over press releases: not needed; evidence is structured. Real-time streaming, any hosted or public web UI (the ADR-18 console is local only), global AIS, Iran theater, SAR imagery, vector databases.
...
shadowfleet/
  ...
  shadowfleet/
    ...
    ui_export/{contract.py, bundle.py, fixtures.py, export.py}   # ADR-18
    cli.py
  ui/                # React console (ADR-18); ui/public/ui_data/ is gitignored
  tests/
...
**ADR-15 Label sources (corrected Sep 17).** OFAC: replay yearly SDN change archives (text parser for 2022 to 2023, PDF parser for 2024 onward), cross-checked against the entry-event dates in the current SDN advanced XML and OpenSanctions `us_ofac_sdn` listing dates. EU: vessels under Annex XLII to Reg 833/2014 (OpenSanctions program `EU-MARE`; Phase 0 records which dataset slug carries them and whether a listing date is present). UK: `gb_fcdo_sanctions` vessels with `listingDate`. For EU and UK, the listing date is cross-checked by diffing dated OpenSanctions exports (monthly snapshots back to about July 2021); disagreement over 31 days is reported per vessel. Revisit if: EU entries lack listing dates and snapshots are unavailable (then EU dates come from the Official Journal amendment dates, parsed by hand for the handful of packages in the window).
...
**ADR-18 Analyst console (added Sep 17, Adam's call).** Options: Streamlit (the plan's original stretch item); React + FastAPI; React as a static app over exported JSON. Choice: React 19 + TypeScript + Vite + deck.gl as a static app over a JSON bundle written by `shadowfleet.ui_export` (`make ui-fixtures` for synthetic data, `make ui-export` for live data). No API server, no database, no tile server: the basemap is Natural Earth 1:10m land (public domain) bundled through `world-atlas`, so the console works offline. Why: Adam wants a console that is satisfying to use and that he will style himself, which Streamlit cannot deliver; a static bundle keeps Python as the only place numbers are made (rule 4) and keeps the backend free of web code. Contract: pydantic models in `ui_export/contract.py` are the source of truth; `make ui-schema` writes JSON Schema to `ui/src/contract/schema/` plus a tiny committed synthetic sample, the console generates its TypeScript types from the schema, and both `pytest` and `vitest` fail on drift. The models enforce project rules at export time: UTC timestamps, `observed_at >= start` (rule 1), no watchlist row for a hull listed on or before its cutoff (rule 3), ordered ranks. The console shows every hull as knowable at an as-of instant that cannot pass the cutoff unless the viewer turns on hindsight, which also reveals outcomes and designations. Licence: dossiers carry GFW-derived events, so the bundle is gitignored and the console is local only (rule 7); a synthetic bundle is permanently badged SYNTHETIC. Cost: one session after Phase 6 (Phase C) to write `export_live`, taken from the week 11 to 12 buffer. Revisit if: bundle size exceeds about 200 MB (then dossiers load per hull from DuckDB through a small read-only endpoint), or Phase 6 slips past week 9 (then the console stays on fixtures and the README uses static figures).
...
| C | Console wiring (ADR-18) | `export_live` fills the frozen UI contract from Phase 2 to 6 outputs; `make ui-export` and `make ui-check` pass; screenshots for the README | Live bundle validates; spot-check 5 hulls against `reports/flagged_*.csv`; no GFW data in git | 10 |
...
Week mapping at 12 to 15 h/week: Phase 0 (wk 1), 1 (wk 1-2, mostly machine time), 2 (wk 2), 3 (wk 3), 4a (wk 4), 4b (wk 4-5), 5a (wk 5-6), 5b (wk 6), 6 (wk 7), 8 (wk 8), 9 (wk 9), 10 (wk 10), C console wiring (wk 11, can run any time after 6), buffer (wk 12); 7 only if the buffer is intact at week 8. The console shell itself was built against synthetic data on Sep 17, in parallel with the Phase 0 ingest.
